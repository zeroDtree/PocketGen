...
Default protocol (PocketGen co-design): fold the generated sequence with ESMFold
and compare it to the generated whole-protein structure.

Optional --sequence_source proteinmpnn uses Genie's ProteinMPNN (8 sequences)
and keeps the sample with the lowest scRMSD.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sequence_source", type=str, choices=("codesign", "proteinmpnn"), default="codesign")
...
def pocket_and_whole_rmsd(generated_pdb: str, predicted_pdb: str, designed_resseq: Sequence[int]) -> Tuple[float, Optional[float], Optional[float]]:
    ...
    pocket_plddt = None
    if designed_resseq:
        gen_p = ca_coords(select_residues_by_resseq(gen_res, designed_resseq))
        pred_p = select_residues_by_resseq(pred_res, designed_resseq)
        pred_ca_p = ca_coords(pred_p)
        m = min(len(gen_p), len(pred_ca_p))
        if m:
            pocket = compute_rmsd(pred_ca_p[:m], gen_p[:m])
...
def evaluate_one_structure(
    ...
    for i, seq in enumerate(sequences):
        ...
        whole, pocket, pocket_plddt = pocket_and_whole_rmsd(generated_pdb, pred_pdb, designed_resseq)
...
def main() -> None:
    ...
    per_sample = []
    for row in tqdm(rows, desc="Designability"):
        cid = row["complex_id"]
    ...
    designable_flags = [float(item["designable"]) for item in per_sample]
    summary = {
        "sequence_source": args.sequence_source,
        "designability": mean_std(designable_flags)["mean"],
        "scTM": mean_std([item["scTM"] for item in per_sample]),
        "scRMSD_whole": mean_std([item["scRMSD_whole"] for item in per_sample]),
        "scRMSD_pocket": mean_std([item["scRMSD_pocket"] for item in per_sample]),
        "pLDDT_pocket": mean_std([item["pLDDT_pocket"] for item in per_sample]),
        "delta_scTM": mean_std([item["delta_scTM"] for item in per_sample]),
        "n": len(per_sample),
    }
    payload = {"summary": summary, "per_sample": per_sample}
    out_json = args.out_json or os.path.join(sample_dir, "designability.json")
